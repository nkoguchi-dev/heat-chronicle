...
function parseParams(): UrlParams {
  ...
  const station = stationRaw !== null && /^\d+$/.test(stationRaw) ? Number(stationRaw) : null;
  const pref = prefRaw !== null && /^\d+$/.test(prefRaw) ? Number(prefRaw) : null;
  const type = typeRaw !== null && VALID_TEMP_TYPES.includes(typeRaw as TempType) ? (typeRaw as TempType) : 'max';
