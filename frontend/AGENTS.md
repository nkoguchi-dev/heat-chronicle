...
- Canvasヒートマップは約200年分の気温を色で俯瞰する視覚的な時系列探索を目的とするため、セル単位の文字ラベル、個別フォーカス、スクリーンリーダー向け代替一覧は要件にしない

## Canvas描画

- 約200年分の気温データを扱うため、データ、表示範囲、寸法など描画結果に関係する値が変わった場合だけCanvasを再描画する
